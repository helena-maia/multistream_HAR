...
def class_count(ground_truth, num_classes):
    count = [ground_truth[ground_truth==i].size for i in range(num_classes)]

def class_acc2(dataset, settings, npy_paths):
    num_classes = 101 if dataset == "ucf101" else 51
    accum_acc = np.zeros(num_classes, dtype=float)
    ...
    for s in range(1,4):
        test_path = os.path.join(settings, "%s/test_split%s.txt" % (dataset, s))
        _, ts_labels = get_labels(test_path)
        y_pred = np.load(npy_paths[s-1])
        conf_matrix = confusion_matrix(ts_labels, y_pred)
        match_count = np.diagonal(conf_matrix)
        ground_truth = np.array(ts_labels)
        class_count = np.array([ground_truth[ground_truth==i].size for i in range(num_classes)])

        accum_acc += match_count / class_count
